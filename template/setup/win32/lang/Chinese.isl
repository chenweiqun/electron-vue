...
MissingWOW64APIs=您正在运行的Windows版本不具备执行64位安装的功能。要解决此问题，请安装补丁 %1。
...
InvalidDrive=您选择的驱动盘或UNC共享不存在或不可访问。请选择其它驱动盘或UNC共享。
...
ApplicationsFound=下列应用程序正在使用运行。建议您允许安装程序自动关闭这些应用程序
ApplicationsFound2=以下应用程序正在使用运行。建议您允许安装程序自动关闭这些应用程序。安装完成后，安装程序将尝试重新启动应用程序。
CloseApplications=&关闭正在运行中的应用程序
DontCloseApplications=&不关闭正在运行中的应用程序
ErrorCloseApplications=安装程序无法自动关闭正在运行的应用程序,请手动关闭应用程序,按下面的重试按钮继续安装
...
ErrorRegisterServerMissingExport=未发现DllRegisterServer的导出接口
